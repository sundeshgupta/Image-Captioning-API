...
def encode(_image):
    __image = preprocess(_image) # preprocess the image
    fea_vec = model_new.predict(__image) # Get the encoding vector for the image
...
def findCaption(image_path):
	pic = encode(image_path)
	image = pic.reshape((1,2048))
	caption = greedySearch(image)
	return caption


# print(findCaption('your_file_name.jpg'))